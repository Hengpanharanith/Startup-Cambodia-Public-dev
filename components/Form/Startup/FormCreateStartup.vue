...
<template>
  ...
  >
    <v-card rounded="lg">
      <v-card-title flat>
        {{ $t("get_listed.startup.title") }} <v-spacer></v-spacer>
        <v-btn
          @click="closeForm"
          tabindex="-1"
          color="primary"
      ...
      <v-card-text class="px-0">
        <v-stepper
          ...
          flat
        >
          <v-form ref="formCreateStartup"
          id="formCreateStartup"
          >
            <v-stepper-items>
                ...
                <v-col
                 ...
                 >
                   <v-autocomplete
                     ...
                     v-model="editedItem.employee_range"
                     item-text="name"
                     item-value="name"
                   >
                   </v-autocomplete>
                 ...
                 >
                   <v-text-field
                       dense
                       outlined
                       v-model="editedItem.number_of_founder"
                       :label="$t('number_of_founders')"
                       ref="number_of_founder"
                 ...
                 >
                   <v-text-field
                       dense
                       outlined
                       v-model="editedItem.number_of_female"
                       :label="$t('female_founders')"
                       ref="number_of_female"
                       :rules="$formRules.numberRule"
                       v-mask="'####'"
                 ...
                 >
                   <v-text-field
                       dense
                       outlined
                       v-model="editedItem.number_of_tech_founder"
                       :label="$t('tech_founders')"
                       ref="number_of_female"
                       :rules="$formRules.number_of_tech_founder"
                       v-mask="'####'"
              ...
              <v-stepper-content :step="4">
                ...
                <v-row v-if="editedItem.number_of_founder">
                  ...
                  <v-col cols="8">{{ editedItem.number_of_founder }} <span class="mx-1"></span> {{ $t("people")}}</v-col>
                </v-row>
                <v-row v-if="editedItem.number_of_female">
                  <v-col cols="4">{{
                      $t("female_founders")
                    }}</v-col>
                  <v-col cols="8">{{ editedItem.number_of_female }} <span class="mx-1"></span> {{ $t("people")}}</v-col>
                </v-row>
                <v-row v-if="editedItem.number_of_tech_founder">
                  <v-col cols="4">{{
                      $t("tech_founders")
                    }}</v-col>
                  <v-col cols="8">{{ editedItem.number_of_tech_founder }} <span class="mx-1"></span>{{ $t("people")}}</v-col>
                </v-row>
      ...
      <v-card-actions>
        ...
        <v-btn
          ...
          color="primary"
          depressed
        >{{stepper == 4 ? isMatched ? $t("get_listed.form.btn.claim") : $t("get_listed.form.btn.create"): $t("get_listed.form.btn.next")}}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    saveForm(nextStep) {
      let valid = this.$refs.formCreateStartup.validate();
    ...
    closeForm() {
      ...
      this.imageUrl = "";
      this.imageFile = "";
      this.$refs.formCreateStartup.reset();
    },
    getLookingAtById(id) {
    ...
    submitForm(startupId = 0) {
      ...
      if(this.isMatched){
        ...
        const formData = new FormData(document.getElementById("formCreateStartup"));
        ...
        if (this.editedItem.employee_range) {
          formData.append("employee_range", this.editedItem.employee_range);
        }

        if (this.editedItem.number_of_founder) {
          formData.append("number_of_founder", this.editedItem.number_of_founder);
        }

        if (this.editedItem.number_of_female) {
          formData.append("number_of_female", this.editedItem.number_of_female);
        }

        if (this.editedItem.number_of_tech_founder) {
          formData.append("number_of_tech_founder", this.editedItem.number_of_tech_founder);
        }
    ...
    _claimProfile(startupId) {
      // 
      this.claimRequest({
        type: "startup",
        stakeholder: startupId,
        option: "claim",
      })
          .then((res) => {
  ...
  computed: {
    ...
    ...mapState("startup-profile", ["isLoading", "getStartupProfile"]),
  },
};
</script>
