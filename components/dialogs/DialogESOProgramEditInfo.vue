...
export default {
  ...
  computed: {
    ...
    modalLabel() {
      ...
      return this.esoProgram.product.id
  ...
  data() {
    return {
      profileImage: null,
      esoProgram: {}, // for loading profile into the view
      startup: {},
      fullPage: true,
      value: [],
      options: [],
  ...
  methods: {
    ...
    openForm(item) {
      this.dialog = true;
      this.esoProgram = this.$common.cloneJSON(item);
      this.imageUrl = this.esoProgram.logo;
    ...
    saveBasicInfo() {
      ...
      this.editProgram({payload: formData, id: this.esoProgram.id})
          .then((res) => {
