...
export default {
  ...
  computed: {
    ...
    programTypesList() {
      return [
        ...
        {
          id: 2,
          name: this.$i18n.locale === 'km' ? "កម្មវិធីអន្តរជាតិ" : "International",
          name_km: "អន្តរជាតិ",
          code: "GLO",
  ...
  methods: {
    ...
    openForm(item) {
      ...
      this.programDisplay = this.esoCohort.program_display;
      // this.startDate = this.programDisplay.start_date;
      // this.endDate = this.programDisplay.end_date;
      this.programDisplay.category = this.programDisplay?.category?.id;
      this.programDisplay.program_type = this.programDisplay?.program_type?.id;

      this.programDisplay.dates = [this.programDisplay.start_date, this.programDisplay.end_date]
      this.editorText = this.programDisplay.content
      this.imageUrl = this.esoCohort?.program_display?.image;
      this.dialog = true;
    },
    ...
    saveBasicInfo() {
      ...
      this.updateCohort({payload: formData, id: this.esoCohort.id})
          .then((res) => {
            this.$swal.fire(
